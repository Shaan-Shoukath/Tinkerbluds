...
| [06_supabase_overlap.md](06_supabase_overlap.md)           | **Supabase Integration & Overlap Detection** — Farmer DB (CRUD by phone), plot storage (GeoJSON + KML), Shapely-based overlap detection (30% threshold), admin alerts, confirmation UI flow, `mailto:` report button, SQL schema, API endpoints                 |

---

## 🧩 Modules Not Yet Documented

The following modules have inline documentation only:

| Module                   | Location                                            | What It Does                                                                                                                                                                                     |
| ------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Yield Service**        | `plot_validation/yield_service.py`                  | 20-crop Kerala-specific dataset, Open-Meteo weather API integration (last 90 days), parameter-by-parameter comparison (temp, rainfall, humidity, NDVI), yield estimation and crop recommendation |
| **NDVI Gradient Mask**   | `earth_engine_service.py` → `generate_thumbnails()` | Gradient vegetation mask — maps NDVI intensity to green brightness (dark = bare, dim = sparse, bright = dense vegetation), clipped to polygon boundary                                           |
